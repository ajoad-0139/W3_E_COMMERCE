    ...
    <div className="w-full h-[650px] flex gap-2">
        {
            products?.length?
                products.map((product)=><div key={product.id} className="flex-1 h-full relative ">
                    <Image src={product.images?.length?product.images[0]:""} loading="eager" alt="hero product image" width={800} height={800} className="w-full h-full object-cover rounded-md"/>
